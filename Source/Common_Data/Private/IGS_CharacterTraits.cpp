...
FIGS_CharacterTraits::FIGS_CharacterTraits() {
    ...
    (*gen2->ContainerPtrToValuePtr<TArray<FGameplayTag>>(&(*this).AllowedWieldables, 0)) = TArray<FGameplayTag>{FGameplayTag::RequestGameplayTag(TEXT("Item.Wieldable"))};
    auto gen3 = TBaseStructure<FGameplayTagQuery>::Get()->FindPropertyByName("QueryTokenStream");
    (*gen3->ContainerPtrToValuePtr<TArray<uint8>>(&(*this).AllowedWieldables, 0)) = TArray<uint8>{0, 1, 2, 1, 0};
